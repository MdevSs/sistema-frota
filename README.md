...
Sistema completo para gestão de motoristas, veículos, rotas e entregas, com integração ERP e modo de testes simplificado.
...
- PostgreSQL (2 instâncias: Logística e ERP)
...
```bash
# Bancos de Dados
DATABASE_URL_LOGISTICA=postgresql://usuario:senha@host:5432/logistica
DATABASE_URL_ERP=postgresql://usuario:senha@host:5432/salutem
...
# Criar tabelas no banco (Drizzle)
pnpm db:push
...
### 👤 Motoristas
- Cadastro completo com validação de CPF/CNH
- Gestão de status (ativo/inativo)

### 🚗 Veículos
- Gestão de frota (VUC, VAN, CAMINHAO)
- Controle de capacidade e dimensões

### 🗺️ Rotas e Entregas
- Criação de rotas com múltiplos pedidos
- Integração com banco ERP para busca de pedidos
...
### 1. Erro: "Banco de dados não disponível"
**Causa:** Variáveis de ambiente incorretas ou banco inacessível.
**Solução:**
- Verifique `DATABASE_URL_LOGISTICA` e `DATABASE_URL_ERP` no `.env`
- Teste a conexão: `psql $DATABASE_URL_LOGISTICA -c "SELECT 1"`
- Verifique se o firewall permite acesso na porta 5432

### 2. Erro: "OAuth callback failed" ou Problemas de Login
**Causa:** Erro na configuração do servidor OAuth.
**Solução:**
- O sistema está em **Modo MVP** por padrão (sem login obrigatório)
- Se o erro persistir, verifique se `server/_core/trpc.ts` está com as proteções comentadas
- Limpe o cache do navegador (LocalStorage/Cookies)

### 3. Erro: "Tabela não encontrada"
**Causa:** Migrations não foram executadas.
**Solução:**
- Execute: `pnpm db:push`
- Verifique no banco: `\dt` no psql

### 4. Erro: "Porta 3000 já em uso"
**Causa:** Outro processo ocupando a porta.
**Solução:**
- Altere `PORT=3001` no `.env`
- Ou mate o processo: `fuser -k 3000/tcp`

### 5. Erro ao buscar pedidos no ERP
**Causa:** Schema do ERP não coincide com o esperado.
**Solução:**
- Verifique `server/routers/erp.ts` e ajuste os nomes das tabelas (`pedido`, `nfenotas`, etc.)
...
- `GUIA_CONFIGURACAO_PRODUCAO.md` - Configuração passo-a-passo
- `CHECKLIST_IMPLEMENTACAO.md` - Todas as funcionalidades
- `MVP_SEM_OAUTH.md` - Como funciona o modo sem login
- `RESUMO_EXECUTIVO.md` - Visão geral do projeto

---

**Desenvolvido para Gestão de Frota e Logística.**
red' | { type: 'function', function: { name: string } }
 * tools?: Tool[]
 */
